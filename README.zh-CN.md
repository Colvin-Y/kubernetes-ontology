...
- 基于标准 Helm labels/annotations 的 HelmRelease 与 HelmChart 归属证据

Helm 关系会以 `managed_by_helm_release` 和 `installs_chart` 边呈现，
...
- 持久化图数据库、外部图后端、运行时 RDF/OWL 物化不在当前 MVP 范围内。
